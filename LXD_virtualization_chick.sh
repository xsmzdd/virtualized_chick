...
check_china
rm -rf log
lxc init images:debian/12 "$1" -c limits.cpu=1 -c limits.memory=256MiB
## 硬盘大小
...
#    storage_type="btrfs"
#fi
#lxc storage create "$1" "$storage_type" size=5GB >/dev/null 2>&1  # 修改硬盘大小为5GB
#lxc config device override "$1" root size=5GB  # 修改硬盘大小为5GB
#lxc config device set "$1" root limits.max 5GB  # 修改硬盘大小为5GB
## IO
...
## 网速
#lxc config device override "$1" eth0 limits.egress=1024Mbit  # 修改带宽为1024Mbit
#lxc config device override "$1" eth0 limits.ingress=1024Mbit  # 修改带宽为1024Mbit
#lxc config device override "$1" eth0 limits.max=1024Mbit  # 修改带宽为1024Mbit
## cpu
